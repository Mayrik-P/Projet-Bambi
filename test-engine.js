...
const enemy138bis = createCar("Adversaire", CAR_SIZE.SMALL, 3, 1); // dans l'arc avant d'une arrivée en (2,2) seulement — PAS en (2,3), la 1ère trajectoire explorée par la recherche brute ("front,front")
...
const enemy138ter = createCar("Adversaire", CAR_SIZE.SMALL, 2, 3); // exactement 2 colonnes devant, MÊME rangée que "front,front" (le 1er chemin exploré) — jamais tirable depuis une arrivée à distance égale (2 colonnes), donc discrimine vraiment le SLAM plutôt qu'une coïncidence de tir
const allCars138ter = [car138ter, enemy138ter];
const path138ter = chooseAiMoveTrajectory(board138ter, car138ter, 2, allCars138ter);
console.log("Chemin choisi (attendu ['front','front'], fonce sur l'adversaire) :", path138ter.join(","));
...
function makeRoundStateForCommand(playerNames, dicePool, commandUsed = {}) {
...
console.log("Placement trouvé, targetCar dans son arc avant (attendu col4 row2) :", cmd153.placement.col, cmd153.placement.row);
...
board158.grid[2][3].hazard = HAZARD_TYPES.MINE; // 1ère case candidate (col3,row2) invalidée par un hazard
...
console.log("Case trouvée = la 3e candidate (attendu col3 row4) :", placement158.col, placement158.row);
