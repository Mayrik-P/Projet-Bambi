...
const carForEntry180 = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
const row180 = chooseAiEntryRow(board180, carForEntry180, [], []);
...
const carForEntry181 = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
const row181 = chooseAiEntryRow(board181, carForEntry181, [occupant181], []);
...
const carForEntry182 = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
const row182 = chooseAiEntryRow(board182, carForEntry182, [], [chopper182]);
console.log("Rangée choisie ≠ 0 (attendu true, évite le chopper) :", row182 !== 0);

section("Test 182bis — chooseAiEntryRow() : si toutes les rangées libres sont épuisées, préfère une case occupée par un ADVERSAIRE plutôt que sa propre équipe (précisé par Mayrik)");

let board182bis = createBoard(createTestTile(8, 6), createTestTile(8, 6), createTestTile(8, 6));
const carForEntry182bis = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
// Toutes les rangées occupées sauf aucune libre : 5 par sa PROPRE équipe (Mayrik), 1 par un adversaire.
const ownTeam182bis = [0, 1, 2, 3, 4].map((row) => createCar("Mayrik", CAR_SIZE.SMALL, 0, row));
const enemyOnRow5 = createCar("Adversaire", CAR_SIZE.SMALL, 0, 5);
const row182bis = chooseAiEntryRow(board182bis, carForEntry182bis, [...ownTeam182bis, enemyOnRow5], []);
console.log("Rangée choisie = celle de l'adversaire, PAS celle de sa propre équipe (attendu 5) :", row182bis);

section("Test 182ter — searchAiFallbackTrajectory() (via chooseAiMoveTrajectory) : au palier de repli, préfère slammer un ADVERSAIRE plutôt que sa propre équipe (repéré par Mayrik en observant une vraie partie)");

const boardFallback = createTestTile(8, 6);
boardFallback.grid[2][1].terrain = TERRAIN.IMPASSABLE; // front-left (col1,row2) bloqué : force le repli
const carFallback = createCar("Mayrik", CAR_SIZE.MEDIUM, 0, 3);
const ownBlockerFallback = createCar("Mayrik", CAR_SIZE.SMALL, 1, 3); // front (col1,row3) : sa propre équipe
const enemyBlockerFallback = createCar("Adversaire", CAR_SIZE.SMALL, 1, 4); // front-right (col1,row4) : adversaire
const pathFallback = chooseAiMoveTrajectory(boardFallback, carFallback, 1, [carFallback, ownBlockerFallback, enemyBlockerFallback], []);
console.log("Chemin choisi (attendu ['front-right'], slam sur l'adversaire) :", pathFallback.join(","));
...
// -----------------------------------------------------------------
// TESTS 193-194 : Drift utilisable sur un tour d'ENTRÉE aussi (repéré
// par Mayrik : oubli initial — chooseAiCommand désactivait Drift pour
// toute voiture pas encore entrée, et même une fois réactivé,
// moveCarEnteringBoard ignorait le paramètre).
// -----------------------------------------------------------------

section("Test 193 — chooseAiCommand() : Drift choisi pour une entrée quand TOUTES les rangées de la colonne 0 sont occupées");

const board193 = createBoard(createTestTile(8, 6), createTestTile(8, 6), createTestTile(8, 6));
const blockers193 = [0, 1, 2, 3, 4, 5].map((row) => createCar("Bloqueur", CAR_SIZE.SMALL, 0, row));
const carOff193 = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
const rs193 = createRoundState(["Mayrik", "Bloqueur"], { Mayrik: [4, 3, 6, 2] });
const cmd193 = chooseAiCommand(board193, carOff193, [carOff193, ...blockers193], [], rs193.dicePool, "Mayrik", rs193);
console.log("Type (attendu 'drift') :", cmd193.type);

section("Test 194 — moveCarEnteringBoard() : Drift appliqué réellement à l'entrée (pas de slam, pas d'élimination)");

tile = createTestTile(8, 6);
const blocker194 = createCar("Bloqueur", CAR_SIZE.SMALL, 0, 3);
const carOff194 = createCarOffBoard("Mayrik", CAR_SIZE.MEDIUM);
const enter194 = moveCarEnteringBoard(tile, carOff194, 4, 3, [], [carOff194, blocker194], { driftAvailable: true });
console.log("ok (attendu true) :", enter194.ok);
console.log("Statut (attendu 'operable', pas éliminée) :", carOff194.status);
console.log("Log mentionne Drift (attendu true) :", enter194.log.some((l) => l.includes("Drift")));

section("Test 195 — chooseAiAssign() / playOneAiTurn() : combo Repair (sur une AUTRE voiture) + entrée en jeu le même tour (repéré par Mayrik)");

const prog195 = makeStandardProgState();
const inoperable195 = createCar("Mayrik", CAR_SIZE.LARGE, 5, 3);
inoperable195.damageTokens = ["a", "b"];
inoperable195.status = CAR_STATUS.INOPERABLE;
inoperable195.movedThisRound = true;
const carOff195 = createCarOffBoard("Mayrik", CAR_SIZE.SMALL);
const round195 = createRoundState(["Mayrik"], { Mayrik: [4, 3, 6, 2] });
const turn195 = playOneAiTurn(prog195, round195, [inoperable195, carOff195], [], ["Mayrik"]);
console.log("ok (attendu true) :", turn195.ok);
console.log("Voiture réparée redevenue operable (attendu true) :", inoperable195.status === CAR_STATUS.OPERABLE);
console.log("Nouvelle voiture bien entrée (col non-null, attendu true) :", carOff195.col !== null);

// -----------------------------------------------------------------
// TESTS 196-198 : p.7, confirmé par Mayrik (jeu physique) — Mine et
// Wreck sont réellement défaussés, mais Blank/Dirt/Oil Slick restent
// EN PLACE sur le plateau, face visible, pour le reste de la partie.
// Seul Oil Slick a un effet supplémentaire (la glissade) qui doit se
// REDÉCLENCHER à chaque passage futur, indéfiniment — contrairement
// à Blank/Dirt dont tout l'effet est déjà capturé par le terrain
// permanent de la case.
// -----------------------------------------------------------------

section("Test 196 — Oil Slick : un 2e passage (voiture différente) sur la même case redéclenche bien la glissade");

tile = createTestTile(8, 6);
tile.grid[3][1].hazard = HAZARD_TYPES.OIL_SLICK;
const oilCar196a = createCar("Mayrik", CAR_SIZE.MEDIUM, 0, 3);
const result196a = moveCar(tile, oilCar196a, 1, ["front"], [oilCar196a], { forcedDice: { oilSlickDirection: "front" } });
console.log("Après 1er passage — hazard (attendu null) :", tile.grid[3][1].hazard);
console.log("Après 1er passage — revealedHazard (attendu 'oil_slick') :", tile.grid[3][1].revealedHazard);

const oilCar196b = createCar("IA", CAR_SIZE.SMALL, 0, 3);
const result196b = moveCar(tile, oilCar196b, 1, ["front"], [oilCar196a, oilCar196b], { forcedDice: { oilSlickDirection: "front" } });
console.log("2e passage : log mentionne la glissade re-déclenchée (attendu true) :", result196b.log.some((l) => l.includes("roule à nouveau sur une case Oil Slick déjà révélée")));
console.log("2e voiture bien glissée elle aussi (attendu col 2, row 3 avant slam en chaîne) :", result196b.log.some((l) => l.includes("est projetée en front vers (col 2, row 3)")));

section("Test 197 — Blank/Dirt : la case reste transformée, mais AUCUN effet supplémentaire ne se redéclenche (juste le coût de terrain normal)");

tile = createTestTile(8, 6);
tile.grid[3][1].hazard = HAZARD_TYPES.DIRT;
const dirtCar197a = createCar("Mayrik", CAR_SIZE.MEDIUM, 0, 3);
moveCar(tile, dirtCar197a, 2, ["front", "front"], [dirtCar197a]); // libère la case après l'avoir révélée
console.log("Après 1er passage — terrain (attendu 'mud') :", tile.grid[3][1].terrain);
console.log("Après 1er passage — revealedHazard (attendu 'dirt') :", tile.grid[3][1].revealedHazard);

const dirtCar197b = createCar("IA", CAR_SIZE.SMALL, 0, 3);
const result197b = moveCar(tile, dirtCar197b, 3, ["front"], [dirtCar197a, dirtCar197b]);
console.log("2e passage : AUCUNE ligne 'déclenche un hazard' (attendu true, terrain seul suffit) :", !result197b.log.some((l) => l.includes("déclenche un hazard")));
console.log("2e passage : coût de 2 bien payé pour la boue (dé 3, attendu remaining 1) :", result197b.remaining);

section("Test 198 — HAZARD_BEHAVIOR : table de référence cohérente (persist pour Blank/Dirt/Oil Slick, discard pour Mine/Wreck)");
const { HAZARD_BEHAVIOR } = require("./engine.js");

console.log("Blank (attendu 'persist') :", HAZARD_BEHAVIOR[HAZARD_TYPES.BLANK]);
console.log("Dirt (attendu 'persist') :", HAZARD_BEHAVIOR[HAZARD_TYPES.DIRT]);
console.log("Oil Slick (attendu 'persist') :", HAZARD_BEHAVIOR[HAZARD_TYPES.OIL_SLICK]);
console.log("Mine (attendu 'discard') :", HAZARD_BEHAVIOR[HAZARD_TYPES.MINE]);
console.log("Wreck (attendu 'discard') :", HAZARD_BEHAVIOR[HAZARD_TYPES.WRECK]);

// -----------------------------------------------------------------
// TESTS 199-200 : garde-fou primaire — chosenPath (mouvement
// VOLONTAIRE, joueur ou IA) ne doit jamais entrer sur une case
// Impassable, même quand une glissade Oil Slick en cours de route a
// repositionné la voiture sur une case que le chemin pré-calculé
// n'avait jamais vérifiée. Bug réel repéré par Mayrik en observant le
// rejeu pas à pas de la Brique 03bis (voiture IA "suicidée" sur une
// case Impassable après un double repositionnement Oil Slick).
// -----------------------------------------------------------------
section("Test 199 — moveCar() : une glissade Oil Slick en cours de route ne doit JAMAIS faire continuer chosenPath vers une case Impassable");

tile = createTestTile(8, 6);
tile.grid[2][3].hazard = HAZARD_TYPES.OIL_SLICK; // devant la voiture (front depuis col2,row2)
tile.grid[2][5].terrain = TERRAIN.IMPASSABLE; // 2e case du chemin, jamais vérifiée par l'IA au moment du choix initial
const car199 = createCar("Test", CAR_SIZE.MEDIUM, 2, 2);
const result199 = moveCar(tile, car199, 3, ["front", "front"], [car199], { forcedDice: { oilSlickDirection: "front" } });

console.log("ok (attendu true) :", result199.ok);
console.log("Voiture PAS éliminée (attendu 'operable') :", car199.status);
console.log("Position finale : glissée puis arrêtée avant l'Impassable (attendu col 4, row 2) :", car199.col, car199.row);
console.log("Log mentionne l'arrêt volontaire avant l'Impassable (attendu true) :", result199.log.some((l) => l.includes("interrompt son mouvement avant d'entrer sur une case impassable")));

section("Test 200 — moveCar() : le garde-fou ne s'applique PAS aux entrées légitimes (Mine, Shrapnel...) — celles-ci passent par forceMoveOneSpace, jamais par chosenPath");

tile = createTestTile(8, 6);
tile.grid[2][3].terrain = TERRAIN.IMPASSABLE; // adjacente (un seul pas), directement atteinte par le déplacement forcé
const car200 = createCar("Test", CAR_SIZE.MEDIUM, 2, 2);
forceMoveOneSpace(tile, car200, [car200], "front"); // simule un jeton Shrapnel/Skid poussant directement dessus
console.log("Élimination normale toujours possible pour un déplacement FORCÉ (attendu 'eliminated') :", car200.status);

section("Test 201 — Blast Off (avec progressionState) : atterrissage hors rangée (gauche/droite) doit ÉLIMINER la voiture, pas juste conserver sa position");

{
  const rear201 = createTestTile(4, 6);
  const middle201 = createTestTile(4, 6);
  const lead201 = createTestTile(4, 6);
  const state201 = createTileProgressionState(rear201, middle201, lead201, []);
  const board201 = buildBoardFromProgressionState(state201);
  const car201 = createCar("Test", CAR_SIZE.MEDIUM, 5, 2);
  const result201 = resolveDamageToken(board201, [car201], car201, "blast_off", {
    progressionState: state201,
    allChoppers: [],
    forcedDice: { blastOffDirection: "front-left", blastOffStunt: 4 } // 4 pas en diagonale → row hors 0..5
  });
  console.log("Log :", result201.log[result201.log.length - 1]);
  console.log("Voiture ÉLIMINÉE (attendu 'eliminated') :", car201.status);
}

section("Test 202 — Slam qui projette la voiture percutée hors du bord ARRIÈRE (col < 0) → élimination");

{
  const tile202 = createTestTile(8, 6);
  const top202 = createCar("Mayrik", CAR_SIZE.LARGE, 0, 3); // col 0 = tout à l'arrière du plateau
  const bottom202 = createCar("IA", CAR_SIZE.SMALL, 0, 3);
  const cars202 = [top202, bottom202];
  const slamResult202 = resolveSlam(tile202, cars202, top202, bottom202, { forcedDice: { slam: "bottom", direction: "rear" } });
  slamResult202.log.forEach((l) => console.log("  " + l));
  console.log("Statut de bottom202 (attendu 'eliminated', projetée hors du bord arrière) :", bottom202.status);
}

section("Test 203 — Blast Off (avec progressionState) : atterrissage hors du bord ARRIÈRE (col < 0) doit ÉLIMINER la voiture");

{
  const rear203 = createTestTile(4, 6);
  const middle203 = createTestTile(4, 6);
  const lead203 = createTestTile(4, 6);
  const state203 = createTileProgressionState(rear203, middle203, lead203, []);
  const board203 = buildBoardFromProgressionState(state203);
  const car203 = createCar("Test", CAR_SIZE.MEDIUM, 1, 2); // proche du bord arrière
  const result203 = resolveDamageToken(board203, [car203], car203, "blast_off", {
    progressionState: state203,
    allChoppers: [],
    forcedDice: { blastOffDirection: "rear", blastOffStunt: 3 } // 3 pas vers l'arrière → col < 0
  });
  console.log("Log :", result203.log[result203.log.length - 1]);
  console.log("Voiture ÉLIMINÉE (attendu 'eliminated') :", car203.status);
}

section("Test 204 — moveCarWithProgression() : un SLAM qui projette la voiture percutée hors du bord AVANT doit décaler la tuile et la repositionner, pas la laisser figée sur place");

{
  const rear204 = createTestTile(4, 6);
  const middle204 = createTestTile(4, 6);
  const lead204 = createTestTile(4, 6);
  const state204 = createTileProgressionState(rear204, middle204, lead204, [createTestTile(4, 6)]);

  const mover204 = createCar("P1", CAR_SIZE.LARGE, 10, 2);
  const occupant204 = createCar("P2", CAR_SIZE.SMALL, 11, 2); // dernière colonne de la lead tile
  const allCars204 = [mover204, occupant204];

  const result204 = moveCarWithProgression(
    state204, mover204, 1, ["front"], allCars204, [], ["P1", "P2"],
    {
      forcedDice: {
        slam: "bottom", // c'est l'occupant (bottom) qui bouge
        direction: "front" // → col 12, hors des 12 colonnes du plateau (0..11) → sortie avant
      }
    }
  );
  result204.log.forEach((l) => console.log("  " + l));
  console.log("ok (attendu true) :", result204.ok);
  console.log("Occupant PAS empilé avec Mover (positions différentes, attendu true) :", mover204.col !== occupant204.col || mover204.row !== occupant204.row);
  console.log("Occupant PAS éliminé (attendu 'operable') :", occupant204.status);
  console.log("Occupant repositionné après décalage de tuile (attendu col 8, row 2) :", occupant204.col, occupant204.row);
}

section("Test 205 — forceMoveOneSpace() centralisé : un déplacement forcé PAR UN DÉGÂT (Skid), pas seulement un Slam direct, doit aussi décaler la tuile en cas de collision en chaîne sortant par l'avant");

{
  const rear205 = createTestTile(4, 6);
  const middle205 = createTestTile(4, 6);
  const lead205 = createTestTile(4, 6);
  const state205 = createTileProgressionState(rear205, middle205, lead205, [createTestTile(4, 6)]);
  const board205 = buildBoardFromProgressionState(state205);

  const shooter205 = createCar("Shooter", CAR_SIZE.LARGE, 9, 2);
  const target205 = createCar("Target", CAR_SIZE.SMALL, 10, 2); // dans l'arc avant du shooter (1 case devant)
  const stationary205 = createCar("Stationary", CAR_SIZE.MEDIUM, 11, 2); // dernière colonne du plateau
  const allCars205 = [shooter205, target205, stationary205];

  // Le tir touche Target, qui reçoit un jeton Skid (direction "front")
  // forcé — Target glisse sur Stationary → slam en chaîne → force le
  // dé de slam pour que ce soit STATIONARY (bottom) qui bouge, et le
  // pousse hors du bord avant du plateau à 12 colonnes.
  const shootResult205 = resolveShoot(board205, allCars205, shooter205, target205, {
    progressionState: state205,
    allChoppers: [],
    forcedDice: {
      shootingDie: "small-medium",
      drawnToken: { type: "skid", skidDirection: "front" },
      slam: "bottom",
      direction: "front"
    }
  });
  shootResult205.log.forEach((l) => console.log("  " + l));
  console.log("Target PAS empilée avec Stationary (attendu true) :", target205.col !== stationary205.col || target205.row !== stationary205.row);
  console.log("Stationary PAS éliminée (attendu 'operable') :", stationary205.status);
