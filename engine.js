...
function moveCarEnteringBoard(tile, car, dieValue, entryRow, chosenPath = [], allCars = [], slamOptions = {}) {
  ...
  const log = [];
  const entryStep = enterAdjacentSpace(tile, car, allCars, 0, entryRow, dieValue, slamOptions, false);
...
function chooseAiCommand(board, actingCar, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  // N/A pour un tour d'ENTRÉE (actingCar.col === null) : il n'existe
  // pas encore d'arc avant tant que la voiture n'est pas sur le
  // plateau — Drift ne s'applique jamais à ce cas.
...
function chooseAiAssign(board, progressionState, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  // p.5-6 : entrée en jeu — prioritaire et court-circuite TOUTE la
  // logique de menace/priorité ci-dessous, qui suppose une position
  // réelle (car.col numérique) pour chaque voiture considérée. Une
  // voiture pas encore entrée (col === null) n'a jamais de Coast
  // possible (elle ne peut pas avoir déjà bougé ce round), donc ce
  // cas ne peut survenir que côté "eligibleCars = neverMoved". Choix
  // déterministe : la première voiture pas encore entrée dans l'ordre
  // du tableau (pas de préférence de taille particulière demandée).
...
function playOneAiTurn(state, roundState, allCars, allChoppers, playerNames) {
  ...
  if (assignDecision.isEntry) {
    ...
    const enterTurnResult = playTurnAssignEnterWithProgression(
      ...
      { roundNumber: roundState.roundNumber, roadDieValue: roundState.roadDie, roadBonusPath }
    );
