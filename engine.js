...
//
// LIMITATION ACTUELLE, à corriger quand le vrai contenu des tuiles
// sera modélisé : pas de "retournement" de tuile (chaque tuile
// physique a 2 faces différentes dans le vrai jeu — nos tuiles de
// test sont à face unique pour l'instant) ; pas de placement
// automatique de hazards sur la nouvelle tuile lead (les cases
// marquées par des triangles rouges ne sont pas encore distinguées
// des autres cases dans notre modèle de tuile).
function createTileProgressionState(rearTile, middleTile, leadTile, drawPile = []) {
  return {
    rearTile,
    middleTile,
    leadTile,
...
function advanceBoardOnFrontExit(state, allCars, allChoppers, options = {}) {
  ...
  // 7. Hazards aléatoires sur la nouvelle lead — PAS ENCORE MODÉLISÉ
  // (nos tuiles de test n'ont pas de marquage de case hazard distinct
  // des autres cases). À faire une fois le vrai contenu des tuiles
  // modélisé.
  log.push(`Nouvelle tuile lead posée (placement automatique des hazards : pas encore modélisé).`);
...
// PAS ENCORE APPELÉE dans le moteur : préparée pour l'étape 7 de
// advanceBoardOnFrontExit() (placement automatique de hazards sur la
// nouvelle tuile lead), qui n'est pas encore implémentée faute de
// distinguer les cases marquées "hazard" des autres dans notre
// modèle de tuile actuel. À utiliser le jour où cette fonctionnalité
// sera codée.
