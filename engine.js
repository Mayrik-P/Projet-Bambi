...
function enterAdjacentSpace(tile, car, allCars, targetCol, targetRow, remaining, slamOptions = {}) {
  const log = [];
...
function moveCar(tile, car, dieValue, chosenPath, allCars = [], slamOptions = {}) {
  ...
  for (const direction of chosenPath) {
    ...
    const step = enterAdjacentSpace(tile, car, allCars, target.col, target.row, remaining, slamOptions);
    log.push(...step.log);
...
// -----------------------------------------------------------------
// 10. MOTEUR DE TOUR — ÉTAPE 1 : ASSIGN + MOVE + END OF TURN (p.8, p.11)
// -----------------------------------------------------------------
...
function playTurnAssignMove(tile, car, dieValue, chosenPath, allCars, allChoppers, options = {}) {
  ...
  // --- END OF TURN ---
  // p.11 : la voiture assignée ne peut plus être assignée ce round,
  // que son mouvement se soit bien déroulé ou non (le dé est "dépensé"
  // dès l'ASSIGN, pas conditionné à la réussite du MOVE).
