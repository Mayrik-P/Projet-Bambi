...
function chooseAiMoveTrajectory(board, car, dieValue, allCars) {
  const tiers = [
  ...
  for (const tier of tiers) {
    ...
    return pickPreferredTrajectory(candidates, slamCandidates, car, allCars);
  }
...
function findAiAirstrikePlacement(board, targetCar, allCars, allChoppers) {
  const candidates = [
    { col: targetCar.col - 1, row: targetCar.row - 1 },
    { col: targetCar.col - 1, row: targetCar.row },
    { col: targetCar.col - 1, row: targetCar.row + 1 }
  ];
...
function chooseAiCommand(board, actingCar, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  for (const enemyOwner of enemyOwners) {
    ...
    const frontmostEnemy = enemyCars.reduce((best, c) => (c.col > best.col ? c : best));
  ...
  if (!otherNeverMoved) {
    ...
    for (const c of allCars) {
      if (c.owner !== playerName && c.status !== CAR_STATUS.ELIMINATED && (!bestEnemy || c.col > bestEnemy.col)) bestEnemy = c;
    }
    if (bestEnemy) {
      const lowestDie = Math.min(...myPool);
...
function chooseAiAssign(board, progressionState, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  const rearmost = (cars) => cars.reduce((best, c) => (c.col < best.col ? c : best));
  const frontmost = (cars) => cars.reduce((best, c) => (c.col > best.col ? c : best));
  ...
  if (myQueueCars.length > 0) {
    ...
    if (enemyThreat) {
      return { car: rearmost(myQueueCars), dieValue: highestDie, command: null };
    }
  }
  ...
  if (progressionState.finishLineTile) {
    const front = frontmost(eligibleCars);
  ...
  if (!isCoastTurn && !roundState.commandUsedThisRound[playerName] && myPool.includes(6)) {
    ...
    if (inoperableCars.length > 0) {
      ...
      if (remainingAfterSix.length > 0) {
        const repairTarget = rearmost(inoperableCars);
        const carToMove = rearmost(eligibleCars);
...
function playOneAiTurn(state, roundState, allCars, allChoppers, playerNames) {
  ...
  if (command && !isCoastTurn) {
    ...
    if (command.type === "nitro") {
      ...
      slamOptions.driftAvailable = true;
  ...
  if (isCoastTurn) {
    const coastPath = chooseAiMoveTrajectory(board, car, 1, allCars);
  ...
  const path = chooseAiMoveTrajectory(board, car, effectiveDieValue, allCars);
  ...
  const turnResult = playTurnAssignMoveWithProgression(
    ...
    { roundNumber: roundState.roundNumber, shootTarget, ...slamOptions }
  );
  log.push(...(turnResult.log || []));
