...
// critères.
function searchAiSafeTrajectory(board, car, allCars, remainingPoints, currentCol, currentRow, pathSoFar, allowedTerrains, avoidHazard, forbidLateralEdgeFinish) {
  if (remainingPoints === 0) {
    if (forbidLateralEdgeFinish && (currentRow === 0 || currentRow === board.rows - 1)) return null;
    return pathSoFar;
  }
  for (const dir of AI_FORWARD_DIRECTIONS) {
    ...
    const result = searchAiSafeTrajectory(board, car, allCars, remainingPoints - cost, targetCol, targetRow, [...pathSoFar, dir], allowedTerrains, avoidHazard, forbidLateralEdgeFinish);
...
function chooseAiMoveTrajectory(board, car, dieValue, allCars) {
  ...
  for (const tier of tiers) {
    const result = searchAiSafeTrajectory(board, car, allCars, dieValue, car.col, car.row, [], tier.terrains, tier.avoidHazard, tier.forbidEdge);
    if (result) return result;
  }
