...
function chooseAiEntryRow(board, allCars, allChoppers = []) {
  const rows = [];
  for (let row = 0; row < board.rows; row++) {
    const space = getSpace(board, 0, row);
    if (!space || space.terrain === TERRAIN.IMPASSABLE) continue;
    const occupied = !!getCarAt(allCars, 0, row) || isChopperOccupied(allChoppers, 0, row);
    const hidden = isAiHiddenHazard(space);
    const terrainRank = space.terrain === TERRAIN.ROAD ? 0 : space.terrain === TERRAIN.OFF_ROAD ? 1 : 2; // MUD
    rows.push({ row, occupied, hidden, terrainRank });
  }

  if (rows.length === 0) return 0; // filet de sécurité, ne devrait jamais arriver (voir commentaire ci-dessus)

  // Tri stable par priorité : d'abord non-occupée, puis sans hazard
  // caché, puis meilleur terrain, puis rangée la plus basse (ordre
  // déterministe en cas d'égalité totale).
  rows.sort((a, b) => {
...
function resolveHazard(tile, allCars, car, remaining, options = {}) {
  ...
  switch (hazardType) {
    ...
    case HAZARD_TYPES.OIL_SLICK: {
      ...
      // de la glissade peut ELLE-MÊME contenir un hazard imbriqué (ex.
      // une Mine), résolu à l'intérieur de forceMoveOneSpace. Sans
      // cette vérification, ce cas tombait tout droit dans le "continue
      // normalement" ci-dessous, ignorant totalement qu'un Mine (ou
      // toute autre résolution imbriquée) venait de mettre la voiture
      // hors service ou de la pousser hors du plateau en cascade — la
      // voiture continuait alors son mouvement D'ORIGINE comme si de
      // rien n'était, parfois jusqu'à sortir du plateau alors qu'elle
      // venait de devenir inopérable.
...
function playOneAiTurn(state, roundState, allCars, allChoppers, playerNames) {
  ...
  if (assignDecision.isEntry) {
    const entryRow = chooseAiEntryRow(board, allCars, allChoppers);
