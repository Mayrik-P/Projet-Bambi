...
// -----------------------------------------------------------------
{
  ...
  // Correctif issu de la revue qualitative v3 (Cas 3, confirmant un
  // 1er signal du lot v2 Cas 6) : instruction explicite de Mayrik —
  // interdire à l'IA de finir un round avec un dé inutilisé si elle
  // a la possibilité de l'utiliser sur une Command.
  const carLast = createCar("A", CAR_SIZE.MEDIUM, 5, 2);
  const enemyForFallback = createCar("B", CAR_SIZE.SMALL, 8, 2);
  const noCommandCandidate = { car: carLast, moveDieValue: 3, moveDieIndex: 0, command: null };
  const ctxLastCarSpareDie = { car: carLast, allCars: [carLast, enemyForFallback], eligibleCars: [carLast], pool: [3, 3] };
  const ctxLastCarNoSpareDie = { car: carLast, allCars: [carLast, enemyForFallback], eligibleCars: [carLast], pool: [3] };
  const ctxOtherCarsRemain = { car: carLast, allCars: [carLast, enemyForFallback], eligibleCars: [carLast, createCar("A", CAR_SIZE.LARGE, 0, 0)], pool: [3, 3] };
  assert(FACTORS.commandParElimination(noCommandCandidate, ctxLastCarSpareDie) < 0,
    "commandParElimination : pas de Command pénalisé en fin de round si un dé utile resterait inutilisé");
  assert(FACTORS.commandParElimination(noCommandCandidate, ctxLastCarNoSpareDie) === 0,
    "commandParElimination : pas de malus si aucun dé ne reste (rien à gaspiller)");
  assert(FACTORS.commandParElimination(noCommandCandidate, ctxOtherCarsRemain) === 0,
    "commandParElimination : pas de malus si d'autres voitures restent éligibles ce round (le dé pourra encore servir)");

  // Correctif issu du conflit découvert au Cas 3 du lot v3 (nouveau lot
  // avec bandeau d'info) : le malus "Nitro non justifié" (Correctif 1)
  // était plus fort que le malus "dé gaspillé en fin de round"
  // (Correctif 3), si bien qu'entre les deux, l'IA préférait quand
  // même gaspiller le dé plutôt que jouer un Nitro non justifié —
  // violant l'instruction de Mayrik. Un Nitro non justifié doit être
  // BEAUCOUP moins pénalisé quand c'est le dernier tour normal du
  // round (l'alternative étant un dé pur perdu), tout en restant moins
  // bon qu'un Nitro réellement justifié.
  const unjustifiedNitroLastTurn = { car: carN, moveDieValue: 1, command: { type: "nitro", dieValue: 1 }, simulated: { finalCol: 6, finalRow: 2, eliminated: false, slam: null } };
  const ctxLastTurnNoTarget = { allCars: [carN], canReachLeadExit: () => false, eligibleCars: [carN] };
  const ctxMidRoundNoTarget = { allCars: [carN], canReachLeadExit: () => false, eligibleCars: [carN, createCar("A", CAR_SIZE.LARGE, 0, 0)] };
  const scoreLastTurn = FACTORS.commandParElimination(unjustifiedNitroLastTurn, ctxLastTurnNoTarget);
  const scoreMidRound = FACTORS.commandParElimination(unjustifiedNitroLastTurn, ctxMidRoundNoTarget);
  assert(scoreLastTurn > scoreMidRound,
    "commandParElimination : Nitro non justifié BEAUCOUP moins pénalisé au dernier tour normal du round qu'en cours de round");
  assert(scoreLastTurn > FACTORS.commandParElimination(noCommandCandidate, ctxLastCarSpareDie),
    "commandParElimination : au dernier tour, un Nitro non justifié doit quand même battre l'absence totale de Command (dé gaspillé)");
}
...
// -----------------------------------------------------------------
// 18. DÉTERMINISME — correctif critique (revue qualitative v5, Cas 2
// signalé par Mayrik) : simulateMoveCandidate() appelait les vraies
// fonctions d'exécution du moteur (moveCar/moveCarEnteringBoard), qui
// résolvent RÉELLEMENT les dés de Slam/Direction/Dégâts via
// Math.random() en interne — deux évaluations du MÊME candidat, sur
// les MÊMES objets, à l'identique, pouvaient renvoyer deux scores
// différents. Corrigé en forçant tous les jets internes via le
// mécanisme forcedDice déjà prévu dans engine.js. Scénario ci-dessous
// force volontairement un Slam (voiture adverse placée exactement sur
// la case d'arrivée) pour exercer le chemin de code le plus à risque.
// -----------------------------------------------------------------
{
  const board = simpleBoard(24, 6);
  const mine = createCar("A", CAR_SIZE.LARGE, 4, 2);
  const enemy = createCar("B", CAR_SIZE.SMALL, 5, 2); // pile sur la case d'arrivée avec dé=1 (front)
  const allCars = [mine, enemy];
  const dicePool = { A: [1, 3] };
  const state = { rearTile: board, middleTile: board, leadTile: board, finishLineTile: null };
  const roundState = { roundNumber: 2, roadDie: 2, commandUsedThisRound: { A: false } };

  const results = [];
  for (let i = 0; i < 10; i++) {
    // Clones frais à chaque appel : le test doit détecter une
    // instabilité du CALCUL lui-même, pas une mutation accidentelle
    // d'un objet partagé entre les appels.
    const freshMine = createCar("A", CAR_SIZE.LARGE, 4, 2);
    const freshEnemy = createCar("B", CAR_SIZE.SMALL, 5, 2);
    const decision = chooseAiAssignCommand(board, state, [freshMine, freshEnemy], [], { A: [1, 3] }, "A", { roundNumber: 2, roadDie: 2, commandUsedThisRound: { A: false } });
    results.push(JSON.stringify({ car: decision.car.size, die: decision.dieValue, cmd: decision.command, score: decision._debug.score }));
  }
  const allIdentical = results.every((r) => r === results[0]);
  assert(allIdentical, "déterminisme : 10 évaluations indépendantes du même scénario (avec Slam forcé) donnent EXACTEMENT le même résultat");
}

// -----------------------------------------------------------------
// 19. DESTINATION — demande explicite de Mayrik : pouvoir juger un
// dé/une Command en connaissant la case réellement visée par l'IA,
// pas seulement le dé lui-même.
// -----------------------------------------------------------------
{
  const board = simpleBoard(24, 6);
  const car = createCar("A", CAR_SIZE.SMALL, 0, 2);
  ...
  const dicePool = { A: [3] };
  const roundState = { roundNumber: 2, roadDie: 2, commandUsedThisRound: { A: false } };
  const decision = chooseAiAssignCommand(board, state, [car], [], dicePool, "A", roundState);
  assert(decision.destination !== null, "destination : présente pour un candidat de mouvement normal");
  assert(decision.destination.col === 3 && decision.destination.row === 2,
    "destination : coordonnées correctes pour un mouvement en ligne droite de 3 cases");
  assert(typeof decision.destination.slam === "boolean" && typeof decision.destination.eliminated === "boolean",
    "destination : expose bien les indicateurs slam/eliminated en plus des coordonnées");
}
