    ...
    if (!moveResult.frontExit) {
      ...
      if (slamExit && slamExit.car.status !== CAR_STATUS.ELIMINATED) {
        ...
        while (retryFrontExit && guard < 4 && exitingCar.status !== CAR_STATUS.ELIMINATED) {
          ...
          const retryResult = forceMoveOneSpace(retryBoard, exitingCar, allCars, exitDirection, movementOptions);
...
function moveCarEnteringBoard(tile, car, dieValue, entryRow, chosenPath = [], allCars = [], slamOptions = {}) {
  ...
  const entryStep = enterAdjacentSpace(tile, car, allCars, 0, entryRow, dieValue, slamOptions, !!slamOptions.driftAvailable);
  ...
  if (entryStep.remaining > 0 && chosenPath.length > 0) {
    const restResult = moveCar(tile, car, entryStep.remaining, chosenPath, allCars, { ...slamOptions, startedInStartingArea: true });
...
function pickPreferredTrajectory(candidates, slamCandidates, car, allCars, allChoppers) {
  ...
  // CORRECTIF (revue qualitative Mayrik, Cas 7 du 2e lot Assign+Command) :
  // plusieurs trajectoires "sûres" de même longueur (même nombre de
  // pas, donc jugées à égalité par le reste de la fonction) peuvent en
  // réalité atteindre des colonnes finales TRÈS différentes sur ce
  // plateau en chevron — "front-left"/"front-right" n'avancent pas
  // toujours d'une colonne selon la parité de la rangée de départ,
  // contrairement à "front". Avant ce correctif, chaque niveau de
  // priorité ci-dessous prenait le PREMIER candidat trouvé (via
  // Array.find, dans l'ordre d'exploration DFS de
  // searchAiSafeTrajectory) sans jamais comparer la progression
  // réelle entre plusieurs candidats satisfaisant le même critère —
  // Mayrik a démontré un cas concret où un chemin tout-Road valide
  // avançait 2 colonnes de plus qu'un autre chemin tout-Road tout
  // aussi sûr, que la fonction n'avait jamais considéré. Désormais,
  // au sein de chaque niveau de priorité, on retient le candidat qui
  // maximise la colonne finale atteinte (progression maximale),
  // jamais juste le premier trouvé.
  const mostAdvanced = (list) => {
    if (list.length === 0) return null;
    return list.reduce((best, c) => (c.finalCol > best.finalCol ? c : best));
  };

  // 1. Sans chopper ET permet de tirer — le meilleur des deux mondes,
  // en préférant la progression maximale parmi ces candidats.
  const bestOfBoth = mostAdvanced(candidates.filter((c) => isSafeFromChopper(c) && canShootAt(c)));
  ...
  // éliminé. Progression maximale parmi ces candidats.
  const safeOnly = mostAdvanced(candidates.filter((c) => isSafeFromChopper(c)));
  if (safeOnly) return safeOnly.path;

  // 3. Repli (cas rare : toutes les cases sûres de ce palier ont un
  // chopper dessus) — préférer un tir malgré le risque, progression
  // maximale parmi ces candidats ; sinon un slam ; sinon la meilleure
  // progression disponible tout court.
  const shootFallback = mostAdvanced(candidates.filter((c) => canShootAt(c)));
  if (shootFallback) return shootFallback.path;
  if (slamCandidates && slamCandidates.length > 0) return slamCandidates[0].path;
  const anyCandidate = mostAdvanced(candidates);
  return anyCandidate ? anyCandidate.path : candidates[0].path;
}
...
// p.8-9 : palier de repli (aucune trajectoire sûre trouvée dans les 7
// paliers précédents) — avance le plus loin possible sans jamais
// choisir volontairement de slammer sa PROPRE équipe si une autre
// option existe. Priorité (confirmée par Mayrik) : continuation
// dégagée (inclut déjà toute case avec un hazard non résolu, puisque
// seule la présence d'un VÉHICULE compte comme "occupée" ici) >
// slammer un ADVERSAIRE > slammer sa propre équipe (uniquement en
// tout dernier recours, si rien d'autre n'est possible).
function searchAiFallbackTrajectory(board, car, allCars, remainingPoints, currentCol, currentRow, pathSoFar) {
  ...
  let enemySlamFallbackDir = null;
  let ownTeamSlamFallbackDir = null;
  ...
  for (const dir of AI_FORWARD_DIRECTIONS) {
    ...
    if (cost === null) continue;
    const occupant = getCarAt(allCars, targetCol, targetRow, car);
    if (!occupant && !bestClearContinuation) {
    ...
    } else if (occupant && occupant.owner !== car.owner && !enemySlamFallbackDir) {
      enemySlamFallbackDir = dir; // 1er candidat "j'accepte le SLAM sur un adversaire ici" rencontré
    } else if (occupant && occupant.owner === car.owner && !ownTeamSlamFallbackDir) {
      ownTeamSlamFallbackDir = dir; // dernier recours absolu seulement
    }
  }

  if (bestClearContinuation) return bestClearContinuation;
  if (enemySlamFallbackDir) return [...pathSoFar, enemySlamFallbackDir];
  if (ownTeamSlamFallbackDir) return [...pathSoFar, ownTeamSlamFallbackDir];
...
// p.5-6 : choix de la rangée d'entrée en jeu pour une voiture pas
// encore sur le plateau (col === null). Contrairement à
// chooseAiMoveTrajectory, il n'y a pas de position de départ ni
// d'arc avant à respecter — n'importe laquelle des `board.rows`
// rangées de la colonne 0 est éligible. Priorité (du meilleur au
// pire) :
//   1. Route, sans jeton hazard caché, sans occupant (voiture/chopper)
//   2. Hors-piste, sans jeton hazard caché, sans occupant
//   3. Boue, sans jeton hazard caché, sans occupant (coût 2, ou 1 si
//      le dé assigné vaut exactement 1 — exception déjà gérée par
//      moveCarEnteringBoard, pas ici)
//   4. La meilleure des cases ci-dessus MALGRÉ un hazard caché
//      (mieux qu'une case occupée)
//   5. Repli : n'importe quelle case column 0 non-occupée et non
//      impassable (jamais impassable en pratique à la mise en place,
//      confirmé par Mayrik — le plateau de départ n'en contient pas)
// Retourne un numéro de rangée (jamais null — une entrée est toujours
// possible quelque part en pratique).
function chooseAiEntryRow(board, car, allCars, allChoppers = []) {
  const rows = [];
  for (let row = 0; row < board.rows; row++) {
    ...
    const occupied = !!occupant || isChopperOccupied(allChoppers, 0, row);
    // p.5-6, précisé par Mayrik : si une rangée occupée doit malgré
    // tout être choisie (toutes les autres épuisées), une case
    // occupée par un ADVERSAIRE reste préférable à une occupée par
    // sa PROPRE équipe.
    const occupiedByOwnTeam = !!occupant && occupant.owner === car.owner;
    const hidden = isAiHiddenHazard(space);
    const terrainRank = space.terrain === TERRAIN.ROAD ? 0 : space.terrain === TERRAIN.OFF_ROAD ? 1 : 2; // MUD
    rows.push({ row, occupied, occupiedByOwnTeam, hidden, terrainRank });
  }

  if (rows.length === 0) return 0; // filet de sécurité, ne devrait jamais arriver (voir commentaire ci-dessus)

  // Tri stable par priorité : d'abord non-occupée, puis pas occupée
  // par sa propre équipe (un adversaire reste préférable), puis sans
  // hazard caché, puis meilleur terrain, puis rangée la plus basse
  // (ordre déterministe en cas d'égalité totale).
  rows.sort((a, b) => {
    if (a.occupied !== b.occupied) return a.occupied ? 1 : -1;
    if (a.occupiedByOwnTeam !== b.occupiedByOwnTeam) return a.occupiedByOwnTeam ? 1 : -1;
    if (a.hidden !== b.hidden) return a.hidden ? 1 : -1;
    if (a.terrainRank !== b.terrainRank) return a.terrainRank - b.terrainRank;
    return a.row - b.row;
  });
...
function chooseAiCommand(board, actingCar, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  // p.10 : le chopper ne peut pas tirer au 1er round — placer une
  // Airstrike n'a alors aucun intérêt tactique immédiat (précisé par
  // Mayrik : autant profiter d'un Nitro pour prendre un bon départ).
  // On désactive donc les deux branches Airstrike ci-dessous pour ce
  // round précis ; le reste de la logique (Drift, Nitro) n'a pas
  // besoin de connaître le round, aucun changement nécessaire là.
  const shootingAllowed = roundState.roundNumber !== 1;
  ...
  // case déjà occupée ou Impassable ? Pour un mouvement normal : les
  // 3 cases de l'arc avant. Pour un tour d'ENTRÉE (actingCar.col ===
  // null, pas encore d'arc avant) : TOUTES les rangées de la colonne
  // 0, puisque le choix de rangée y est totalement libre — Drift
  // permet d'entrer sans slammer si chacune est occupée (repéré par
  // Mayrik : l'entrée ne devrait pas être exclue de cette logique).
  if (actingCar.col !== null) {
    ...
    const allEntryRowsBlocked = Array.from({ length: board.rows }, (_, row) => row).every((row) => {
      const space = getSpace(board, 0, row);
      if (!space) return true;
      return space.terrain === TERRAIN.IMPASSABLE || !!getCarAt(allCars, 0, row);
    });
    if (allEntryRowsBlocked) {
      const driftDie = myPool.find((v) => v >= 3 && v <= 5);
      if (driftDie !== undefined) return { type: "drift", dieValue: driftDie };
    }
  }
...
function chooseAiAssign(board, progressionState, allCars, allChoppers, dicePool, playerName, roundState) {
  ...
  // p.5-6 : le combo Repair (ci-dessous, section "1.") reste possible
  // même quand la voiture qui reçoit le dé de mouvement ce tour est
  // justement une voiture pas encore entrée — Repair cible une AUTRE
  // voiture (n'importe laquelle du joueur), donc rien n'empêche de
  // réparer une voiture déjà sur le plateau tout en faisant entrer
  // une nouvelle (repéré par Mayrik : l'entrée ne devrait pas exclure
  // cette combinaison). Vérifié AVANT le court-circuit générique
  // d'entrée ci-dessous, avec la même condition que le vrai bloc
  // Repair plus loin dans cette fonction.
  if (!isCoastTurn && !roundState.commandUsedThisRound[playerName] && myPool.includes(6)) {
    const inoperableCarsEarly = allCars.filter((c) => c.owner === playerName && c.status === CAR_STATUS.INOPERABLE);
    if (inoperableCarsEarly.length > 0) {
      const remainingAfterSixEarly = [...myPool];
      remainingAfterSixEarly.splice(remainingAfterSixEarly.indexOf(6), 1);
      if (remainingAfterSixEarly.length > 0) {
        const repairTargetEarly = findRearmostCar(inoperableCarsEarly);
        const carToMoveEarly = eligibleCars.find((c) => c.col === null) || findRearmostCar(eligibleCars.filter((c) => c.col !== null));
        if (carToMoveEarly) {
          const carDieEarly = Math.max(...remainingAfterSixEarly);
          return {
            car: carToMoveEarly,
            dieValue: carDieEarly,
            command: { type: "repair", dieValue: 6, target: repairTargetEarly },
            isEntry: carToMoveEarly.col === null
          };
        }
      }
    }
  ...
  // p.5-6 : au-delà du combo Repair ci-dessus, une voiture pas encore
  // entrée passe en priorité absolue — court-circuite la logique de
  // menace/priorité ci-dessous, qui suppose une position réelle
  // (car.col numérique) pour chaque voiture considérée. Une voiture
  // pas encore entrée (col === null) n'a jamais de Coast possible
  // (elle ne peut pas avoir déjà bougé ce round), donc ce cas ne peut
  // survenir que côté "eligibleCars = neverMoved". Choix déterministe
  // : la première voiture pas encore entrée dans l'ordre du tableau
  // (pas de préférence de taille particulière demandée).
  const notYetEntered = eligibleCars.find((c) => c.col === null);
  if (notYetEntered) {
    return { car: notYetEntered, dieValue: highestDie, command: null, isEntry: true };
  }
...
function forceMoveOneSpace(tile, car, allCars, directionName, options = {}, _guard = 0) {
  ...
  if (space === undefined) {
    ...
    if (options.progressionState && !options.progressionState.finishLineTile && _guard < 4) {
      ...
      const retryResult = forceMoveOneSpace(advanceResult.newBoard, car, allCars, directionName, options, _guard + 1);
      log.push(...retryResult.log);
...
function resolveHazard(tile, allCars, car, remaining, options = {}) {
  ...
  if (!cell.hazard && cell.revealedHazard === HAZARD_TYPES.OIL_SLICK) {
    ...
    return resolveOilSlickSlide(tile, allCars, car, remaining, options, log);
  }
        ...
        if (occupant2) {
          ...
          if (slamExit2 && slamExit2.car.status !== CAR_STATUS.ELIMINATED) {
            ...
            while (retryFrontExit2 && guard2 < 4 && exitingCar2.status !== CAR_STATUS.ELIMINATED) {
              ...
              const retryResult2 = forceMoveOneSpace(currentBoard, exitingCar2, allCars, slamExit2.direction, options);
...
function playTurnAssignEnterWithProgression(state, car, dieValue, entryRow, chosenPath, allCars, allChoppers, playerNames, options = {}) {
  ...
  const board = buildBoardFromProgressionState(state);
  const enterResult = moveCarEnteringBoard(board, car, dieValue, entryRow, chosenPath, allCars, options);
...
function playOneRandomTurn(state, roundState, allCars, allChoppers, playerNames, options = {}) {
  ...
  if (turnResult.ok) log.push(...advanceTurn(roundState, allCars).log);
...
function playOneAiTurn(state, roundState, allCars, allChoppers, playerNames) {
  ...
  // p.5-6 : sur un tour d'entrée en jeu, on ne tente PAS de Command —
  // simplification volontaire (chooseAiCommand suppose une position
  // réelle sur le plateau pour son évaluation de menace, ce qu'une
  // voiture pas encore entrée n'a pas).
  // p.5-6 : une Command reste possible sur un tour d'entrée (rien ne
  // l'interdit dans les règles) — chooseAiCommand gère déjà le cas
  // d'une voiture pas encore entrée (Drift désactivé proprement, voir
  // son propre commentaire), donc pas de simplification nécessaire ici.
  ...
  if (command && !isCoastTurn) {
    ...
    if (command.type === "nitro") {
      ...
      if (nitroResult.ok) {
        const baseDieValue = effectiveDieValue;
        effectiveDieValue += nitroResult.bonus;
        // Clarification demandée par Mayrik : la ligne "ASSIGN" qui
        // suit affichera directement le total déjà combiné (base +
        // bonus Nitro) — sans cette ligne explicite, ça ressemble à
        // tort à un second dé de mouvement distinct à additionner
        // soi-même, alors que le calcul est déjà fait.
        log.push(`Nitro : ${baseDieValue} (dé de base) + ${nitroResult.bonus} (bonus Nitro) = ${effectiveDieValue} points de mouvement au total`);
      }
  ...
  // p.5-6 : tour d'entrée en jeu — court-circuite le reste (recherche
  // de trajectoire normale, projection pour le tir, etc.), qui
  // suppose tous une position déjà réelle sur le plateau.
  if (assignDecision.isEntry) {
    const entryRow = chooseAiEntryRow(board, car, allCars, allChoppers);
    const entrySpace = getSpace(board, 0, entryRow);

    // Continuation du trajet après l'entrée (jusqu'ici absente : la
    // voiture s'arrêtait net après être entrée, même s'il restait du
    // déplacement) — même principe que pour un mouvement normal,
    // depuis la position hypothétique d'entrée, pour ce qu'il reste
    // après le coût de terrain de la case d'entrée elle-même.
    let entryChosenPath = [];
    if (entrySpace && entrySpace.terrain !== TERRAIN.IMPASSABLE) {
      ...
      if (remainingAfterEntry > 0) {
        const hypotheticalCarAfterEntry = { ...car, col: 0, row: entryRow };
        entryChosenPath = chooseAiMoveTrajectory(board, hypotheticalCarAfterEntry, remainingAfterEntry, allCars, allChoppers);
      }
    }

    // Bonus Road : même principe que le mouvement normal — on estime
    // l'éligibilité (route probable dès l'entrée, cf. chooseAiEntryRow
    // qui la préfère) avant même de savoir si elle se confirmera
    // réellement à l'exécution (roadEligible fait foi côté moteur).
    let roadBonusPath = null;
    if (roundState.roadDie) {
      const entrySpaceForBonus = getSpace(board, 0, entryRow);
      if (entrySpaceForBonus && entrySpaceForBonus.terrain === TERRAIN.ROAD) {
        const hypotheticalCar = { ...car, col: 0, row: entryRow };
        roadBonusPath = chooseAiMoveTrajectory(board, hypotheticalCar, roundState.roadDie, allCars, allChoppers);
      }
    }

    const enterTurnResult = playTurnAssignEnterWithProgression(
    ...
    log.push(...(enterTurnResult.log || []));
    if (enterTurnResult.ok) log.push(...advanceTurn(roundState, allCars).log);
    return { ...enterTurnResult, log };
  }
  ...
  if (isCoastTurn) {
    ...
    if (coastResult.ok) log.push(...advanceTurn(roundState, allCars).log);
...
module.exports = {
  ...
  searchAiSafeTrajectory,
  getForwardDelta,
  AI_FORWARD_DIRECTIONS,
  computeAiStepCost,
  isAiHiddenHazard,
  isAiPathAllRoad,
  chooseAiShootTarget,
...
// parking lot — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_01A = {
  id: "vendetta-01a",
  name: "parking lot",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// runway to ruin — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_01B = {
  id: "vendetta-01b",
  name: "runway to ruin",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// snakeway — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_02A = {
  id: "vendetta-02a",
  name: "snakeway",
  format: "vendetta",
  extension: "base",
  ...
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// unrest stop — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_02B = {
  id: "vendetta-02b",
  name: "unrest stop",
  format: "vendetta",
  extension: "base",
  ...
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// mudhole — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_03A = {
  id: "vendetta-03a",
  name: "mudhole",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// bunker — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_03B = {
  id: "vendetta-03b",
  name: "bunker",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: true }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// barrens — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_04A = {
  id: "vendetta-04a",
  name: "barrens",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};

// scenic route — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_04B = {
  id: "vendetta-04b",
  name: "scenic route",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }]
  ]
};

// route sixty styx — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_05A = {
  id: "vendetta-05a",
  name: "route sixty styx",
  format: "vendetta",
  extension: "base",
  cols: 8,
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: true }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
...
// choke — généré par l'outil de tagging, à relire avant usage
const TILE_VENDETTA_05B = {
  id: "vendetta-05b",
  name: "choke",
  format: "vendetta",
  extension: "base",
  ...
  rows: 6,
  grid: [
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }],
    [{ terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: false }, { terrain: TERRAIN.MUD, hazardSpace: false }, { terrain: TERRAIN.ROAD, hazardSpace: true }, { terrain: TERRAIN.ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }],
    [{ terrain: TERRAIN.OFF_ROAD, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.IMPASSABLE, hazardSpace: false }, { terrain: TERRAIN.OFF_ROAD, hazardSpace: false }]
  ]
};
...
function loadRealTiles() {
  return [TILE_VENDETTA_01A, TILE_VENDETTA_01B, TILE_VENDETTA_02A, TILE_VENDETTA_02B, TILE_VENDETTA_03A, TILE_VENDETTA_03B, TILE_VENDETTA_04A, TILE_VENDETTA_04B, TILE_VENDETTA_05A, TILE_VENDETTA_05B];
}
