...
function dashboardImagePath(playerName, kind) {
  // kind: "command" | "small" | "medium" | "large" | "<size>-inoperable"
...
function dieMarkup(value, color, x, y, size, extraAttrs) {
...
function renderDashboards() {
  ...
  order.forEach((playerName, rowIndex) => {
    ...
    // --- Diceboard : dés du pool de ce joueur, non encore assignés ---
    const pool = G.roundState.dicePool[playerName] || [];
    pool.forEach((value) => {
      const isClickable = playerName === HUMAN && dieStep;
      const dieY = originY + (DASH_H - DIE_SIZE) / 2;
      svg.insertAdjacentHTML("beforeend", dieMarkup(value, PLAYER_CAR_COLOR[playerName], x, dieY, DIE_SIZE, isClickable ? 'class="clickable"' : ""));
      if (isClickable) {
    ...
    ["small", "medium", "large"].forEach((size) => {
      ...
      if (car && !isInoperable && playerName === HUMAN && clickableCarSet.has(car)) {
        const f = VEHICLE_SLOT_FRACTION[size][ctx.mode === "coast" ? (car.coastCount === 0 ? "coast1" : "coast2") : "any"];
        const sx = x + f.x * w - slotSize / 2, sy = originY + f.y * DASH_H - slotSize / 2;
        svg.insertAdjacentHTML("beforeend", `<rect class="clickable" x="${sx.toFixed(1)}" y="${sy.toFixed(1)}" width="${slotSize.toFixed(1)}" height="${slotSize.toFixed(1)}" fill="#b0d458" fill-opacity="0.55" stroke="#b0d458" stroke-width="1.5"/>`);
      ...
      if (car && ctx && sel.car === car) {
        const f = VEHICLE_SLOT_FRACTION[size][ctx.mode === "coast" ? "coast1" : "any"];
        ...
        svg.insertAdjacentHTML("beforeend", dieMarkup(sel.dieValue, PLAYER_CAR_COLOR[playerName], dieX, dieY, slotSize, isCancelable ? 'class="clickable"' : ""));
        if (isCancelable) {
