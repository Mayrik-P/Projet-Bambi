...
function initDashboardsPanzoom() {
  ...
  try {
    ...
    dashboardsPanzoom = Panzoom(svg, { maxScale: 4, minScale: 1, contain: "outside", canvas: true, origin: "0 0" });
    ...
    // BUG CORRIGÉ (retour de Mayrik, usage réel — vue calée sur le
    // CENTRE, pas l'angle haut gauche) : la vraie cause n'était ni
    // focal ni un problème de timing (tenté puis vérifié faux), mais
    // le containment "outside" lui-même — sa formule interne calcule
    // ses bornes avec un terme symétrique (diffHorizontal/2, voir
    // panzoom.js) qui suppose implicitement une mise à l'échelle
    // centrée, INDÉPENDAMMENT de origin:'0 0' — un vrai zoom
    // (scale()+pan(0,0)) qui devrait pourtant fournir un résultat
    // fiable dans les coordonnées de containment ("outside") se
    // retrouve donc reclampé vers une position centrée, malgré un
    // transform-origin correctement posé. Vérifié empiriquement dans
    // un vrai Chromium (Playwright) : pan(0,0) avec containment actif
    // se fait reclamper à une valeur non nulle ; avec contain:false
    // sur CET appel précis (bascule les points de départ ET
    // d'arrivée), le recalage tombe pile sur l'angle haut gauche à
    // n'importe quelle échelle. Contourner le containment est sans
    // risque ici : (0,0) est une position connue et sûre (le contenu
    // couvre toujours la zone visible, jamais d'espace vide), le
    // containment ne protège que contre un DÉPLACEMENT libre de
    // l'utilisateur, pas contre une valeur programmatique déjà connue
    // bonne. contain:false évite aussi le besoin d'un setTimeout
    // (plus de dépendance aux dimensions, donc plus de problème de
    // fraîcheur après le zoom).
    document.querySelectorAll("#dashboards-zoom-presets button").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        dashboardsPanzoom.pan(0, 0, { animate: false, contain: false });
      });
    });
