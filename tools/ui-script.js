...
function renderBoard() {
  ...
  carsInDrawOrder.forEach((car) => {
    ...
    // seule l'orientation change. Plus de trait blanc en travers (ni
    // ici, ni sur une épave) — l'opacité réduite reste le seul
    // traitement visuel de l'inopérabilité.
    const imgCenterX = x + CAR_IMG_W / 2;
    const rotation = car.status === "inoperable" ? `transform="rotate(180 ${imgCenterX.toFixed(1)} ${cy.toFixed(1)})"` : "";
    svg.insertAdjacentHTML("beforeend", `<g>
      <image href="${imgPath}" xlink:href="${imgPath}" x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${CAR_IMG_W.toFixed(1)}" height="${CAR_IMG_H.toFixed(1)}" ${rotation} ${car.status === "inoperable" ? 'opacity="0.5"' : ""}/>
