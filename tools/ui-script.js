  ...
  order.forEach((playerName, rowIndex) => {
    ...
    if (playerName === OPPONENT && cp === OPPONENT && !gameOver && !G.aiPending) {
      ...
      svg.insertAdjacentHTML("beforeend", `<foreignObject x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${btnW.toFixed(1)}" height="${btnH.toFixed(1)}">
        <button xmlns="http://www.w3.org/1999/xhtml" class="primary" style="width:100%;height:100%;font-size:${(btnH * 0.26).toFixed(1)}px;line-height:1.15;white-space:normal;box-sizing:border-box;padding:2px;" ${G.aiAnimating ? "disabled" : ""}>${label}</button>
      </foreignObject>`);
      if (!G.aiAnimating) {
...
function renderBoard() {
  ...
  carsInDrawOrder.forEach((car) => {
    ...
    svg.insertAdjacentHTML("beforeend", `<g>
      ...
      ${isInoperableVisual ? `<image href="${INOPERABLE_MARKER_PATH}" xlink:href="${INOPERABLE_MARKER_PATH}" x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${MARKER_ICON_SIZE.toFixed(1)}" height="${MARKER_ICON_SIZE.toFixed(1)}" pointer-events="none"/>` : (car.damageTokens.length > 0 ? `<image href="${DAMAGE_MARKER_PATH}" xlink:href="${DAMAGE_MARKER_PATH}" x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${MARKER_ICON_SIZE.toFixed(1)}" height="${MARKER_ICON_SIZE.toFixed(1)}" pointer-events="none"/>` : "")}
    </g>`);
  });
  ...
  if (pendingSlam) {
    ...
    if (isOnBoard(board(), dest.col, dest.row)) {
      ...
      svg.insertAdjacentHTML("beforeend", `<image href="${slamPath}" xlink:href="${slamPath}" x="${(dc.cx - MARKER_ICON_SIZE / 2).toFixed(1)}" y="${(dc.cy - MARKER_ICON_SIZE / 2).toFixed(1)}" width="${MARKER_ICON_SIZE.toFixed(1)}" height="${MARKER_ICON_SIZE.toFixed(1)}" class="clickable"/>`);
      svg.lastElementChild.addEventListener("click", () => { resume(true); render(); });
    }
  ...
  if (sel.step === "road-bonus-choice" && sel.car) {
    const front = getFrontArc(sel.car).find((a) => a.name === "front");
    if (front && isOnBoard(board(), front.col, front.row)) {
