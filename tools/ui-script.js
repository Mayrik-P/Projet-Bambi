...
// Même idée d'ombre portée que pour les véhicules, mais implémentée
// différemment (retour de Mayrik après capture d'écran réelle + tour
// d'horizon des techniques standard) : la silhouette d'une voiture
// est arrondie (un simple décalage suffit), alors qu'un jeton hazard
// a de VRAIES pointes (pointe droite, encoche gauche) où la forme a
// une largeur quasi nulle — un décalage seul y sépare complètement
// l'ombre du jeton. `feDropShadow` (l'outil standard pour exactement
// ce cas : ombre qui suit le canal alpha d'une forme irrégulière,
// accéléré par le GPU du navigateur) résout ça nativement : le léger
// flou intégré au filtre borde suffisamment les pointes pour ne
// jamais laisser de vide, sans avoir à dessiner plusieurs copies. Un
// seul filtre, posé directement sur l'image réelle (qui gère aussi la
// fusion avec l'ombre en dessous) — beaucoup plus simple que
// l'ombrage des véhicules (qui, lui, doit dessiner une silhouette
// séparée : feDropShadow n'existait pas encore dans le code à ce
// moment-là, non revu ici pour ne pas toucher à un rendu déjà validé
// par Mayrik).
const HAZARD_SHADOW_OFFSET = HAZARD_IMG_W * 0.05;
const HAZARD_SHADOW_BLUR = HAZARD_IMG_W * 0.006; // volontairement minime : juste assez pour ne pas laisser de vide sur les pointes (retour de Mayrik, préfère un bord net à un flou doux)
const HAZARD_SHADOW_OPACITY = 0.7;
...
function renderBoard() {
  ...
  // Filtre d'ombre portée pour les jetons hazard (voir le commentaire
  // détaillé près de HAZARD_SHADOW_OFFSET) — feDropShadow standard,
  // suit le canal alpha de la forme (donc ses pointes) sans laisser de
  // vide, fusionne lui-même l'ombre et l'image d'origine. Défini UNE
  // SEULE FOIS par rendu, réutilisé par tous les jetons.
  svg.insertAdjacentHTML("beforeend", `<defs><filter id="hazardDropShadow" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="${HAZARD_SHADOW_OFFSET.toFixed(2)}" dy="${HAZARD_SHADOW_OFFSET.toFixed(2)}" stdDeviation="${HAZARD_SHADOW_BLUR.toFixed(2)}" flood-color="#000000" flood-opacity="${HAZARD_SHADOW_OPACITY}"/></filter></defs>`);
  ...
  for (let row = 0; row < b.rows; row++) {
    for (let col = 0; col < b.cols; col++) {
      ...
      if (cell.hazard) {
        ...
        svg.insertAdjacentHTML("beforeend", `<image href="${HAZARD_BACK_PATH}" xlink:href="${HAZARD_BACK_PATH}" x="${hx.toFixed(1)}" y="${hy.toFixed(1)}" width="${HAZARD_IMG_W.toFixed(1)}" height="${HAZARD_IMG_H.toFixed(1)}" filter="url(#hazardDropShadow)" pointer-events="none"/>`);
        ...
        svg.insertAdjacentHTML("beforeend", `<image href="${p}" xlink:href="${p}" x="${hx.toFixed(1)}" y="${hy.toFixed(1)}" width="${HAZARD_IMG_W.toFixed(1)}" height="${HAZARD_IMG_H.toFixed(1)}" filter="url(#hazardDropShadow)" pointer-events="none"/>`);
      }
    }
  }
