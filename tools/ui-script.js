...
function initDashboardsPanzoom() {
  ...
  try {
    const svg = document.getElementById("dashboards");
    // Configuration NATIVE de Panzoom, sans rien de personnalisé
    // (retour de Mayrik) : après plusieurs tentatives pour caler
    // précisément le zoom sur l'angle haut gauche (origin personnalisé
    // + containment maison + inertie), le résultat en usage réel était
    // décevant — marges visibles, glissement qui déborde puis se
    // recentre brutalement avec l'inertie. Mis de côté pour
    // l'instant : ce n'est pas bloquant, seulement un confort d'usage.
