...
// Hauteur d'affichage commune à tous les dashboards d'une même ligne
// (la largeur de chacun se déduit de son propre rapport largeur/
// hauteur natif — jamais une valeur indépendante qui déformerait
// l'image, même principe que pour les tuiles/véhicules).
const DASH_H = 70;
const DASH_GAP = 6;
...
// hauteur AFFICHÉE du dashboard concerné. Première estimation faite à
// l'œil sur l'image de référence envoyée par Mayrik — aucune image
// n'étant visible dans ce bac à sable, PAS encore calée finement. À
// affiner avec un outil de calage dédié une fois vérifié par Mayrik
// dans son navigateur (même méthode que tools/calage.html /
// tools/calage-vehicules.html) — la géométrie (fractions) est isolée
// ici pour rester facile à corriger sans toucher au reste du rendu.
const VEHICLE_SLOT_SIZE_FRACTION = 0.28; // fraction de DASH_H — taille affichée des slots ANY/COAST
const COMMAND_SLOT_SIZE_FRACTION = 0.20; // idem pour les 4 slots du command board
const VEHICLE_SLOT_FRACTION = {
  ...
  // forcément le même agencement imprimé. Valeurs de départ =
  // estimation à l'œil sur l'image de référence de Mayrik, à
  // remplacer par l'export de tools/calage-dashboards.html.
  small: {
    any: { x: 0.50, y: 0.42 },
    endTurn: { x: 0.50, y: 0.68 }, // pas encore câblé (S8, prochaine tranche)
    coast1: { x: 0.86, y: 0.30 },
    coast2: { x: 0.97, y: 0.30 } // arbitraire (aucun repère visuel imprimé, voir spec section 6)
  },
  medium: {
    any: { x: 0.50, y: 0.42 },
    endTurn: { x: 0.50, y: 0.68 },
    coast1: { x: 0.86, y: 0.30 },
    coast2: { x: 0.97, y: 0.30 }
  },
  large: {
    any: { x: 0.50, y: 0.42 },
    endTurn: { x: 0.50, y: 0.68 },
...
const COMMAND_SLOT_FRACTION = {
  ...
  // texte) — géométrie posée ici par anticipation pour la prochaine.
  nitro: { x: 0.86, y: 0.22 },
  drift: { x: 0.14, y: 0.78 },
  repair: { x: 0.86, y: 0.78 },
  airstrike: { x: 0.86, y: 0.06 }
};
...
const DICEBOARD_SLOT_SIZE_FRACTION = 0.55; // fraction de DASH_H — taille affichée des 4 dés sur le diceboard
// 4 emplacements DROITS (retour de Mayrik) — première estimation à
// l'œil (répartition régulière en ligne), à remplacer par l'export de
// tools/calage-dashboards.html comme les autres.
...
function renderDashboards() {
  ...
  order.forEach((playerName, rowIndex) => {
    const originY = rowIndex * (DASH_H + PLAYER_ROW_GAP) + 10;
    let x = 10;
    ...
    svg.insertAdjacentHTML("beforeend", `<image href="${dicePath}" xlink:href="${dicePath}" x="${x.toFixed(1)}" y="${originY.toFixed(1)}" width="${diceW.toFixed(1)}" height="${DASH_H.toFixed(1)}" pointer-events="none"/>`);
    const dieSize = DASH_H * DICEBOARD_SLOT_SIZE_FRACTION;
    diceboardSlots(playerName).forEach((value, i) => {
      if (value === null) return;
      const slotKey = "slot" + i;
      const f = DICEBOARD_SLOT_FRACTION[slotKey];
      const dx = x + f.x * diceW - dieSize / 2, dy = originY + f.y * DASH_H - dieSize / 2;
      const isClickable = playerName === HUMAN && dieStep;
      svg.insertAdjacentHTML("beforeend", dieMarkup(value, PLAYER_CAR_COLOR[playerName], dx, dy, dieSize, isClickable ? 'class="clickable"' : "", SLOT_ROTATION[slotKey]));
      if (isClickable) {
    ...
    svg.insertAdjacentHTML("beforeend", `<image href="${cmdPath}" xlink:href="${cmdPath}" x="${x.toFixed(1)}" y="${originY.toFixed(1)}" width="${cmdW.toFixed(1)}" height="${DASH_H.toFixed(1)}" pointer-events="none"/>`);
    x += cmdW + DASH_GAP;
    ...
    ["small", "medium", "large"].forEach((size) => {
      ...
      const w = dashWidth(native.w, native.h);
      ...
      if (car) {
        ...
        svg.insertAdjacentHTML("beforeend", `<image href="${imgPath}" xlink:href="${imgPath}" x="${x.toFixed(1)}" y="${originY.toFixed(1)}" width="${w.toFixed(1)}" height="${DASH_H.toFixed(1)}" pointer-events="none"/>`);
      }
      // Véhicule éliminé (car === undefined ici) -> emplacement laissé
      // vide, voir spec-dashboards.md section 2.

      const slotSize = DASH_H * VEHICLE_SLOT_SIZE_FRACTION;
      ...
      if (car && !isInoperable && playerName === HUMAN && clickableCarSet.has(car)) {
        ...
        const cx = x + f.x * w, cy = originY + f.y * DASH_H;
        const sx = cx - slotSize / 2, sy = cy - slotSize / 2;
        const rotAttr = SLOT_ROTATION[slotKey] ? ` transform="rotate(${SLOT_ROTATION[slotKey]} ${cx.toFixed(1)} ${cy.toFixed(1)})"` : "";
        svg.insertAdjacentHTML("beforeend", `<rect class="clickable" x="${sx.toFixed(1)}" y="${sy.toFixed(1)}" width="${slotSize.toFixed(1)}" height="${slotSize.toFixed(1)}" fill="#b0d458" fill-opacity="0.55" stroke="#b0d458" stroke-width="1.5"${rotAttr}/>`);
      ...
      if (car && ctx && sel.car === car) {
        ...
        const f = VEHICLE_SLOT_FRACTION[size][slotKey];
        const dieX = x + f.x * w - slotSize / 2, dieY = originY + f.y * DASH_H - slotSize / 2;
        const isCancelable = PRE_COMMIT_STEPS.has(sel.step) && sel.step !== "commit";
        svg.insertAdjacentHTML("beforeend", dieMarkup(sel.dieValue, PLAYER_CAR_COLOR[playerName], dieX, dieY, slotSize, isCancelable ? 'class="clickable"' : "", SLOT_ROTATION[slotKey]));
        if (isCancelable) {
  ...
  const totalH = order.length * (DASH_H + PLAYER_ROW_GAP) + 10;
