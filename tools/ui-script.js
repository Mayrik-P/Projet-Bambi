...
// Face imprimée de chaque type de hazard. Sert à la révélation dans la
// fenêtre Illustration, donc elle couvre les CINQ types — y compris
// Mine et Wreck, dont les images étaient présentes dans le dépôt sans
// jamais s'afficher nulle part, puisque le moteur les défausse ou les
// transforme aussitôt. La fenêtre est la seule occasion de les voir.
const HAZARD_FACE_IMAGE = {
  blank: "../images/hazards/hazard-road.webp",
  dirt: "../images/hazards/hazard-mud.webp",
  oil_slick: "../images/hazards/hazard-oilslick.webp",
  mine: "../images/hazards/hazard-mine.webp",
  wreck: "../images/hazards/hazard-wreck.webp"
};

// Sous-ensemble qui reste POSÉ sur le plateau, face visible, pour le
// reste de la partie (p.7). Deux notions distinctes qu'il ne faut pas
// fusionner malgré le recouvrement : l'une dit « à quoi ressemble ce
// jeton », l'autre « ce jeton reste-t-il sur la case ». Les chemins,
// eux, n'ont qu'une source.
const HAZARD_REVEALED_IMAGE = {
  blank: HAZARD_FACE_IMAGE.blank,
  dirt: HAZARD_FACE_IMAGE.dirt,
  oil_slick: HAZARD_FACE_IMAGE.oil_slick
};
...
//
// ATTENTION — il existe DEUX natures d'attente, et une seule est
// réglable (retour de Mayrik après plusieurs parties) :
//
//   - entre deux étapes que la MACHINE enchaîne toute seule (un pas de
//     plus, des dés qui tombent, un déplacement induit), le battement
//     sert à comprendre ce qui se passe : c'est PACE_MS, réglé par les
//     boutons Lent/Moyen/Rapide, et il est jugé bon tel quel ;
//
//   - entre la fin d'une scène et le moment où le JOUEUR reprend la
//     main (surbrillance des destinations, question de relance de
//     Slam), il n'y a rien à comprendre : c'est une latence. En dessous
//     de 0,1 s, l'utilisateur a le sentiment que c'est son geste qui
//     agit ; au-delà, il a le sentiment d'attendre la machine. D'où
//     zéro ci-dessous.
//
// La séparation existait déjà en creux dans le fichier : apresBattement()
// n'est appelé QUE machine→machine, gelerPendantLaScene() QUE
// machine→joueur. Elles ne partagent simplement plus la même valeur.
const BATTEMENT_RENDU_MAIN_MS = 0;

function attendreRetourDeMain() {
  if (BATTEMENT_RENDU_MAIN_MS <= 0) return null;
  return new Promise((r) => setTimeout(r, BATTEMENT_RENDU_MAIN_MS));
}
...
// durée est à 0.
//
// `dureeMs` permet à un appelant d'imposer une autre durée que le
// réglage Lent/Moyen/Rapide — voir BATTEMENT_PAS_JOUEUR_MS.
function apresBattement(suite, dureeMs) {
  const attente = attendreAnimations();
  const ms = (dureeMs === undefined) ? PACE_MS : dureeMs;
  const battre = () => { if (ms > 0) setTimeout(suite, ms); else suite(); };
  if (attente) attente.then(battre); else battre();
}

// Un pas que le JOUEUR vient de choisir lui-même ne demande aucun temps
// de lecture : il regardait déjà la case au moment de cliquer. Lui
// appliquer le réglage de rythme revenait à le faire attendre avant son
// propre coup suivant — en Lent, une seconde entre chaque case (retour
// de Mayrik, mesuré). Cette durée est donc FIXE, calée sur le ressenti
// du mode Rapide, et volontairement pas indexée sur PACE_MS.
//
// Ce qui, à l'inverse, garde le réglage : tout ce que la machine décide
// sans lui — hazards, jets de dés, Slams, déplacements induits — et les
// pas de l'IA, où voir la voiture avancer case par case est justement
// l'intérêt du mode Lent.
const BATTEMENT_PAS_JOUEUR_MS = 200;
...
function gelerPendantLaScene() {
  ...
  G.uiLocked = true;
  appliquerVoileDeGel();
  attente.then(attendreRetourDeMain).then(() => {
    G.uiLocked = false;
    render();
  });
  return true;
...
// --- SCÈNE ÉCOURTABLE PAR UN TAP -----------------------------------
// Deux natures d'affichage bien distinctes dans la fenêtre
// Illustration, et une seule est concernée ici :
//
//   - SCÈNE FINIE (révélation d'un jeton de dégât) : le joueur n'a
//     rien à décider, ça dure le temps que ça dure. C'est long par
//     construction — il faut le temps de lire le jeton — donc
//     écourtable.
//   - BOUCLE (à venir : Slam en attente de relance, tir en attente de
//     cible) : ça tourne pendant que le joueur choisit sur le plateau,
//     sans durée propre. Rien à écourter, et l'interface doit rester
//     vive puisqu'on attend justement un clic. Une boucle n'appellera
//     donc jamais ce qui suit.
//
// Écourter ne fait RIEN perdre : le moteur a déjà tout résolu quand la
// scène se joue (le rythme est purement visuel), et l'état final de la
// révélation est persistant — le jeton reste face visible sous le
// dashboard tant que ses effets s'appliquent.
//
// Seule la fenêtre Illustration écoute (demande de Mayrik) : un
// attrapeur plein écran avalerait les gestes de défilement et de zoom,
// qui sont désormais les gestes natifs du navigateur.
let sceneIllustration = null;

// #wrap est en position:fixed, donc il crée un contexte d'empilement :
// un z-index sur #illu-module, qui est son enfant, ne peut PAS le faire
// passer au-dessus du voile anti-clic posé sur <body>. On cale donc une
// surcouche fixe indépendante sur le rectangle écran réel de la
// fenêtre — exactement la technique déjà utilisée pour l'animation des
// dés (voir animateOneMovingDie). Elle fonctionne avec ou sans voile,
// ce qui compte : pendant le tour de l'IA il n'y a pas de voile.
function poserAttrapeurDeTap(scene) {
  if (typeof document === "undefined" || !document.body) return;
  ...
  zone.style.zIndex = "9999"; // au-dessus du voile (9998)
...
function ouvrirSceneIllustration() {
  const scene = { passee: false, reprendre: null, t0: performance.now(), retirerAttrapeur: null };
  sceneIllustration = scene;
  poserAttrapeurDeTap(scene);
  return scene;
}

function fermerSceneIllustration(scene) {
  if (scene.retirerAttrapeur) scene.retirerAttrapeur();
  if (sceneIllustration === scene) sceneIllustration = null;
}

// Un tap = TOUTE la scène en cours jusqu'à son état final, pas une
// étape de plus (décision de Mayrik). Les taps ne s'accumulent pas :
// une scène déjà passée ne se repasse pas, et la scène close ne répond
// plus du tout. Le délai d'armement évite qu'un tap arrivé une fraction
// de seconde après la fin d'une scène ne tue la suivante avant même
// qu'elle ne s'affiche — ce qui arrive dans une cascade de dégâts, où
// deux scènes s'enchaînent dans la même image.
const ARMEMENT_SKIP_MS = 150;

function sauterSceneIllustration() {
  const scene = sceneIllustration;
  if (!scene || scene.passee) return false;
  if (performance.now() - scene.t0 < ARMEMENT_SKIP_MS) return false;
  scene.passee = true;
  if (scene.reprendre) scene.reprendre();
  return true;
}

// Attente interruptible : résout au bout de `ms`, ou tout de suite si
// la scène a été écourtée entre-temps.
function pauseDeScene(scene, ms) {
  if (scene.passee || ms <= 0) return Promise.resolve();
  return new Promise((resolve) => {
    const t = setTimeout(() => { scene.reprendre = null; resolve(); }, ms);
    scene.reprendre = () => { clearTimeout(t); scene.reprendre = null; resolve(); };
  });
}
...
//
// `scene` est optionnel : s'il est fourni et que le joueur écourte en
// plein retournement, on saute directement à la face révélée, à plat,
// plutôt que de finir une rotation que personne ne regarde plus.
function animerRetournement(el, faceArriere, totalMs, scene) {
  return new Promise((resolve) => {
    const t0 = performance.now();
    let echange = false;
    function frame(now) {
      if (scene && scene.passee) {
        el.src = faceArriere;
        el.style.transform = "";
        resolve();
        return;
...
// Trois temps, dans un rapport 1/2/4 donné par Mayrik : le dos
// apparaît, le jeton se retourne, la face tirée reste exposée assez
// longtemps pour qu'on comprenne DE QUEL jeton il s'agit et ce qu'il
// fait. Avant ce réglage, tout tenait dans un seul retournement de
// PACE_MS — on ne voyait ni l'animation ni la face.
const REVELATION_TEMPS = { apparition: 500, rotation: 1000, exposition: 2000 };

// Une durée de transition suit la distance parcourue ; un temps de
// LECTURE suit la quantité d'information à comprendre, qui ne change
// pas avec le réglage. La révélation ne suit donc pas l'échelle
// PACE_PRESETS (rapport de 4,5 entre Lent et Rapide, bien trop violent
// pour de la lecture) mais une échelle compressée : même en Rapide,
// l'exposition tient 1,4 s. C'est le plancher qui empêche le mode
// Rapide de devenir un mode « on saute tout » — l'écueil connu des
// portages de jeux de plateau, où l'option « passer les animations »
// finit par rendre le jeu illisible.
const REVELATION_ECHELLE = { slow: 1.3, medium: 1.0, fast: 0.7 };

function dureesRevelation() {
  const k = REVELATION_ECHELLE[paceSpeedName] || 1;
  return {
    apparition: Math.round(REVELATION_TEMPS.apparition * k),
    rotation: Math.round(REVELATION_TEMPS.rotation * k),
    exposition: Math.round(REVELATION_TEMPS.exposition * k)
  };
}

// La scène de révélation, indépendante de ce qu'elle révèle : un dos
// s'installe, se retourne, et la face reste exposée. `onFin` est joué à
// la toute fin — c'est lui, et lui seul, qui dit ce que devient le
// jeton une fois révélé, puisque ça diffère selon les cas :
//   - jeton de DÉGÂT : il va se poser face visible sous le dashboard,
//     où il n'était pas ;
//   - jeton de HAZARD : il est déjà sur sa case, et c'est le moteur qui
//     décide juste après s'il y reste face visible (Road/Mud/Oil Slick)
//     ou s'il disparaît (Mine/Wreck). Il n'y a donc rien à faire ici :
//     la fenêtre se vide, le plateau reprend la main.
// Le moteur attend la fin de cette scène avant de résoudre l'effet
// (elle est inscrite au registre des animations en vol), donc la case
// montre encore son dos pendant toute la révélation — c'est cohérent,
// et ça ne demande aucun séquencement supplémentaire.
function jouerRevelation(dos, face, onFin) {
  const illu = document.getElementById("illu-module");
  if (!illu || !face || !scenePeutAnimer()) { onFin(); return; }
  illu.innerHTML = "";
  const img = document.createElement("img");
  img.src = dos;
  img.alt = "";
  illu.appendChild(img);
  const t = dureesRevelation();
  const scene = ouvrirSceneIllustration();
  suivreAnimation((async () => {
    await pauseDeScene(scene, t.apparition);   // 1 — le dos s'installe
    await animerRetournement(img, face, t.rotation, scene); // 2 — il se retourne
    await pauseDeScene(scene, t.exposition);   // 4 — la face tirée reste lisible
    fermerSceneIllustration(scene);
    onFin();
    illu.innerHTML = "";
    render();
  })());
...
function revelerJeton(token) {
  jouerRevelation(DAMAGE_FRONT, damageImagePath(token), () => jetonsReveles.add(token));
}

// Même principe, même durées, même tap pour écourter : un hazard qu'on
// vient de déclencher se retourne dans la fenêtre avant d'être résolu.
function revelerHazard(hazardType) {
  const face = HAZARD_FACE_IMAGE[hazardType];
  if (!face) return; // type sans visuel connu (extensions à venir) : aucune scène plutôt qu'une fenêtre vide
...
function driveHumanStepGenerator(gen, onComplete, answer) {
  ...
  if (!outcome.done) {
    if (isPresentationEvent(outcome.pending)) {
      ...
      appliquerVoileDeGel();
      render();
      // Le pas de SA propre voiture, qu'il vient de cliquer, repart
      // tout de suite (durée fixe) ; tout le reste — hazard, dés,
      // Slam, déplacement induit, et le pas d'une AUTRE voiture
      // poussée par un Slam — garde le réglage de rythme.
      const sonProprePas = outcome.pending.type === "step" && outcome.pending.car && sel.car && outcome.pending.car === sel.car;
      apresBattement(() => driveHumanStepGenerator(gen, onComplete), sonProprePas ? BATTEMENT_PAS_JOUEUR_MS : undefined);
      return;
    }
    G.uiLocked = false;
