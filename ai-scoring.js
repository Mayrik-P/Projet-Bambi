...
// -----------------------------------------------------------------
// CORRECTIF CRITIQUE (revue qualitative v5, Cas 2 — signalé par
// Mayrik) : simulateMoveCandidate() appelait les vraies fonctions
// d'exécution du moteur (moveCar/moveCarEnteringBoard), qui résolvent
// RÉELLEMENT les dés de Slam/Direction/Dégâts/hazards via
// Math.random() en interne (voir engine.js). Résultat : évaluer DEUX
// FOIS le MÊME candidat, sur les MÊMES objets, à l'identique, pouvait
// renvoyer deux scores différents — l'IA "tirait les dés" pendant sa
// PROPRE RÉFLEXION, avant même que le joueur ait engagé le
// mouvement réel. Ce n'est pas de la variété voulue (bruit
// intentionnel, désactivé par défaut) : c'était un défaut de fond qui
// rendait CHAQUE décision instable, y compris entre deux évaluations
// strictement identiques.
//
// Correctif : on force TOUS les jets de dés internes à des valeurs
// fixes et neutres via le mécanisme forcedDice déjà prévu dans
// engine.js (utilisé pour ses propres 212 tests déterministes) —
// jamais de réimplémentation de la logique de mouvement ici, on
// réutilise le vrai moteur, juste sans hasard. Choix des valeurs :
// - slam="bottom", direction="front" : résultat de Slam simple et
//   prévisible (la case exacte d'atterrissage après un Slam n'est de
//   toute façon pas ce qui compte pour le score — les facteurs
//   utilisent déjà le matchup de taille, pas la position exacte
//   post-Slam).
// - drawnToken=DENT ("no effect other than taking damage", p.12) :
//   le choix le plus neutre possible pour un jeton de dégâts simulé —
//   évite qu'un Shrapnel/Dazed/BlastOff simulé ne redirige la
//   trajectoire de façon totalement arbitraire et disproportionnée
//   par rapport à ce qu'un tir/hazard réel produirait en moyenne.
// - oilSlickDirection="front" : neutre, garde la voiture dans son axe.
const DETERMINISTIC_SIMULATION_DICE = {
  forcedDice: {
    slam: "bottom",
    direction: "front",
    rerolledSlam: "bottom",
    rerolledDirection: "front",
    drawnToken: { type: "dent", skidDirection: null },
    oilSlickDirection: "front",
    shrapnelDirection: "front",
    dazedStunt: 1,
    dazedDirections: ["front", "front", "front", "front"],
    blastOffDirection: "front",
    blastOffStunt: 1
  }
};
...
function simulateMoveCandidate(board, car, effectiveDieValue, allCars, allChoppers) {
  ...
  const result = moveCar(boardClone, carClone, effectiveDieValue, path, carsClone, DETERMINISTIC_SIMULATION_DICE);
  return {
    finalCol: carClone.col,
    finalRow: carClone.row,
    eliminated: carClone.status === CAR_STATUS.ELIMINATED,
...
const WEIGHTS_BY_PROFILE = {
  peloton: {
    ...
    repairPriorite: 2, commandParElimination: 1.2, usureAdverse: 1
  },
  echappee: {
    ...
    repairPriorite: 2, commandParElimination: 1.2, usureAdverse: 0.5
  },
  hybride: {
    ...
    repairPriorite: 2, commandParElimination: 1.2, usureAdverse: 0.8
  }
};
...
function commandParElimination(candidate, ctx) {
  if (!candidate.command) {
    // CORRECTIF (revue qualitative Mayrik, lot v3, Cas 3+ ; déjà
    // observé une 1re fois lot v2 Cas 6) : en fin de round, un dé
    // qui ne servira JAMAIS (plus aucune autre voiture éligible ce
    // round, pas de Coast pertinent) était laissé inutilisé, donc
    // perdu pour rien — les dés sont relancés à neuf chaque round,
    // rien ne se reporte. Instruction explicite de Mayrik : interdire
    // à l'IA de finir un round avec un dé inutilisé si elle a la
    // possibilité de l'utiliser. La logique "réserver la Command
    // pour plus tard" (voir ci-dessous pour Nitro) s'INVERSE ici :
    // s'il n'y a plus de "plus tard" ce round, même une Command
    // imparfaite vaut mieux que le gaspillage total.
    if (ctx.eligibleCars && ctx.eligibleCars.length <= 1) {
      const usedIndices = new Set([candidate.moveDieIndex]);
      const remaining = ctx.pool.filter((_, idx) => !usedIndices.has(idx));
      if (remaining.length > 0) {
        const hasEnemy = ctx.allCars.some((c) => c.owner !== candidate.car.owner && c.status !== CAR_STATUS.ELIMINATED);
        const hasNitroOrDriftDie = remaining.some((v) => v >= 1 && v <= 5);
        const hasRepairDie = remaining.includes(6);
        if (hasEnemy || hasNitroOrDriftDie || hasRepairDie) {
          return -1.8; // dé(s) restants gaspillés en fin de round évitable
        }
      }
    }
    return 0;
  ...
  if (candidate.command.type === "nitro") {
    // CORRECTIF (revue qualitative Mayrik, 10 cas Assign+Command,
    // session du <session en cours>) : la version précédente
    // retournait toujours 0, donc aucun malus réel ne s'appliquait
    // jamais — Nitro était collé au premier mouvement disponible
    // dans 7 cas sur 10, sans justification tactique, la Command du
    // round étant alors gaspillée (parfois au prix de renoncer à un
    // Airstrike clairement meilleur, cf. Cas 3/6). Principe validé
    // par Mayrik : Nitro doit être CALIBRÉ sur un objectif concret
    // (pas juste "plus de mouvement disponible") — un tir qui
    // s'ouvre grâce au surplus, une sortie de tuile atteinte, ou un
    // Slam favorable rendu possible par ce surplus. Sans un de ces
    // trois, le dé de Command est plus utile ailleurs (réservé pour
    // une autre voiture, ou pour Airstrike).
    if (!candidate.simulated || candidate.simulated.eliminated) return -1.2;
    const { finalCol, finalRow } = candidate.simulated;
    const arc = getFrontArc({ col: finalCol, row: finalRow });
    const enablesShot = ctx.allCars.some(
      (c) => c.owner !== candidate.car.owner && c.status !== CAR_STATUS.ELIMINATED &&
        arc.some((a) => a.col === c.col && a.row === c.row)
    );
    const totalMove = candidate.moveDieValue + candidate.command.dieValue;
    const reachesExit = ctx.canReachLeadExit(candidate.car, totalMove);
    let favorableSlam = false;
    if (candidate.simulated.slam) {
      const target = getCarAt(ctx.allCars, finalCol, finalRow, candidate.car);
      favorableSlam = !!target && sizeRank(candidate.car.size) >= sizeRank(target.size);
    }
    if (enablesShot || reachesExit || favorableSlam) return 1.0; // Nitro justifié par un objectif concret

    // CORRECTIF (revue qualitative v3, Cas 3) : sans cet assouplissement,
    // ce malus (-2.2) entrait en conflit avec le malus "dé gaspillé en
    // fin de round" (voir plus haut) — les DEUX malus s'appliquaient
    // simultanément sur le dernier tour normal du round, si bien que
    // "Medium seule" (dé gaspillé) restait quand même préférée à
    // "Medium + Nitro non justifié", ce qui viole l'instruction
    // explicite de Mayrik ("interdire à l'IA de finir un round avec un
    // dé inutilisé si elle a la possibilité de l'utiliser"). Sur le
    // dernier tour normal du round, un Nitro sans objectif clair reste
    // n'empêche PAS d'utiliser une Command : mieux vaut CETTE Command
    // imparfaite que le dé perdu — donc le malus est fortement réduit
    // dans ce contexte précis, tout en restant légèrement négatif pour
    // continuer à préférer un Nitro réellement justifié quand un choix
    // existe.
    if (ctx.eligibleCars && ctx.eligibleCars.length <= 1) return -0.2;
    return -2.2; // Nitro "gratuit" en cours de round : le vrai biais repéré par Mayrik sur 7/10 cas
  }
...
function chooseAiAssignCommand(board, progressionState, allCars, allChoppers, dicePool, playerName, roundState, options = {}) {
  ...
  return {
    ...
    // Destination simulée du candidat retenu — ajoutée pour la revue
    // qualitative avec Mayrik : il jugeait jusqu'ici le dé/la Command
    // choisis SANS connaître la case d'arrivée visée par l'IA, ce qui
    // rendait le jugement partiel (un choix de dé peut être justifié
    // par une destination que Mayrik n'avait pas envisagée). Reflète
    // la simulation DÉTERMINISTE (voir DETERMINISTIC_SIMULATION_DICE)
    // — une position plausible parmi d'autres si un Slam/hazard réel
    // survient en jeu, pas une garantie absolue.
    destination: best.simulated
      ? {
          col: best.simulated.finalCol,
          row: best.simulated.finalRow,
          slam: !!best.simulated.slam,
          eliminated: !!best.simulated.eliminated
        }
      : null,
